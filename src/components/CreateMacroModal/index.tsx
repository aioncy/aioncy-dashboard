...
const CreateMacroModal = ({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        ...
        !availabilityRef.current.contains(e.target as Node)
      ) {
        setIsAvailabilityOpen(false);
      }
    };
  ...
  const availabilityLabel = isNoneSelected
    ? "None"
    : isEveryoneSelected
      ? "Everyone"
          ...
          {isAvailabilityOpen && (
            <div className={styles.availabilityMenu}>
              <div
                ...
                onClick={() =>
                  setAvailableFor(COLLABORATORS.map((c) => c.name))
                }
              >
                <span
                  className={`${styles.availabilityIcon} ${styles.neutralIcon}`}
                >
                  <Users size={12} />
                </span>
              ...
              {COLLABORATORS.map((collaborator) => (
                ...
                >
                  <span
                    ...
                    style={{ background: avatarColor(collaborator.name) }}
                  >
